...
class BaseUser(object):
    def __init__(self, uid, access_key, secret_key, is_sub = False):
        self.uid = uid
        self.secret_key = secret_key
	if is_sub:
	    self.access_key = uid + ':' + access_key
	    self.swift_connection = swiftclient.Connection(user = self.access_key, key = self.secret_key, authurl = 'http://' + ceph_cluster_server + '/auth')
        else:
...
class RGWUser(BaseUser):
    def __init__(self, uid, access_key, secret_key, display_name):
        BaseUser.__init__(self, uid, access_key, secret_key, False)
...
class RGWSubuser(BaseUser):
    def __init__(self, uid, subuser, access_key, secret_key):
        self.key_type = 'swift'
        self.access = 'full'
	BaseUser.__init__(self, uid, access_key, secret_key, True)
        self.subuser = subuser


class RGWOper(object):

    def ls_bucket(self, user, times, is_sub):
	if is_sub:    
            for i in range(times):
                user.swift_connection.get_account()
	else:
	    for i in range(times):
		user.s3_connection.get_all_buckets()

    def create_bucket(self, user, is_sub):
	if is_sub:
	    user.bucket = user.access_key.split(':')[1] + "_bucket"
	    user.swift_connection.put_container(user.bucket)
	else:
	    user.bucket = user.s3_connection.create_bucket(user.access_key + "_bucket")


    def delete_bucket(self, user, is_sub):
	if is_sub:
            user.swift_connection.delete_container(user.bucket)
        else:
...
class Tester(object):
	...
	self.swiftchosenone = RGWSubuser("zvampirem", "swiftchosenone", "swiftchosenone", "swiftchosenone")
	self.swiftchosenoneex = RGWSubuser("zvampirem", "swiftchosenoneex", "swiftchosenoneex", "swiftchosenoneex")
        self.zvampirem2 = RGWUser("zvampirem2", "zvampirem2", "zvampirem2", "zvampirem2")
	self.swiftchosenone2 = RGWSubuser("zvampirem2", "swiftchosenone2", "swiftchosenone2", "swiftchosenone2")
        self.zvampirem3 = RGWUser("zvampirem3", "zvampirem3", "zvampirem3", "zvampirem3")
	self.swiftchosenone3 = RGWSubuser("zvampirem3", "swiftchosenone3", "swiftchosenone3", "swiftchosenone3")
    ...
    def test_preparation(self):
        self.m_rgw_operater.ls_bucket(self.zvampirem, 3, False)
        self.m_rgw_operater.create_bucket(self.swiftchosenone, True)
        self.m_rgw_operater.delete_bucket(self.swiftchosenone, True)
        self.m_rgw_operater.ls_bucket(self.swiftchosenone2, 7, True)
        self.m_rgw_operater.ls_bucket(self.zvampirem3, 6, False)
        self.m_rgw_operater.ls_bucket(self.swiftchosenoneex, 2, True)
...
#            print res_content
    ...
    def test_usage_show_without_any_param(self):
        expect_dict = {"entries_size": 4,
                       "zvampirem": {"": {"categories": {"list_buckets": {"ops": 5, "successful_ops": 5}}},
                                     "swiftchosenone_bucket": {"categories": {"create_bucket": {"ops": 1, "successful_ops": 1}, "delete_bucket": {"ops": 1, "successful_ops": 1}}}},
    ...
    def test_usage_show_with_uid_subuser_for_user_operating(self):
        ...
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem3", subuser = "swiftchosenone3")
        response = requests.get(req_url, auth=S3Auth(ceph_cluster_admin_access_key, ceph_cluster_admin_secret_key, ceph_cluster_server))
#        print response.status_code
        res_content = json.loads(response.text)
    ...
    def test_usage_show_with_uid_subuser_for_subuser_operating(self):
        ...
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem2", subuser = "swiftchosenone2")
        result = self.verify_get_response_msg(req_url, expect_dict)
    ...
    def test_usage_show_with_uid_subuser_without_categories(self):
        expect_dict = {"entries_size": 1,
                       "zvampirem:swiftchosenone": {"swiftchosenone_bucket": {"categories": {"create_bucket": {"ops": 1, "successful_ops": 1}, "delete_bucket": {"ops": 1, "successful_ops": 1}}}}}

        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem", subuser = "swiftchosenone")
        result = self.verify_get_response_msg(req_url, expect_dict)
    ...
    def test_usage_show_with_categories_without_uid(self):
        expect_dict = {"entries_size": 4,
                       "zvampirem": {"": {"categories": {"list_buckets": {"ops": 5, "successful_ops": 5}}},
                                     "swiftchosenone_bucket": {"categories": {}}},
                       "zvampirem2": {"": {"categories": {"list_buckets": {"ops": 7, "successful_ops": 7}}}},
                       "zvampirem3": {"": {"categories": {"list_buckets": {"ops": 6, "successful_ops": 6}}}},
                       "time_test": {"": {"categories": {"list_buckets": {"ops": 4, "successful_ops": 4}}}}}
    ...
    def test_usage_show_with_uid_user_and_categories(self):
        expect_dict = {"entries_size": 1,
                       "zvampirem": {"": {"categories": {}},
                                     "swiftchosenone_bucket": {"categories": {"create_bucket": {"ops": 1, "successful_ops": 1}}}}}
    ...
    def test_usage_show_with_uid_subuser_and_categories(self):
        expect_dict = {"entries_size": 1,
                       "zvampirem:swiftchosenone": {"swiftchosenone_bucket": {"categories": {"create_bucket": {"ops": 1, "successful_ops": 1}}}}}
        
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}&categories={categories}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem", subuser = "swiftchosenone", categories = "create_bucket")
        result = self.verify_get_response_msg(req_url, expect_dict)
    ...
    def test_usage_show_with_start_uid_categories_without_end(self):
        ...
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}&categories={categories}&start={start}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem", subuser = "swiftchosenone", categories="create_bucket", start = self.m_start_time_stamp)
        result = self.verify_get_response_msg(req_url, expect_dict)
...
#        print res_content
    ...
    def test_usage_trim_with_uid_subuser(self):
        ...
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem", subuser = "swiftchosenoneex")
        result = self.verify_del_response_msg(req_url, expect_dict)
    ...
    def test_usage_trim_with_end_and_uid_without_start(self):
        ...
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}&end={end}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem", subuser = "swiftchosenone", end = self.m_end_time_stamp)
        result = self.verify_del_response_msg(req_url, expect_dict)
    ...
    def test_usage_trim_with_start_end_and_uid(self):
        ...
        req_url = "http://{server}/{admin}/usage?format=json&uid={uid}&subuser={subuser}&start={start}&end={end}".format(server = ceph_cluster_server, admin = ceph_cluster_admin, uid = "zvampirem", subuser = "swiftchosenone", start = self.m_start_time_stamp, end = self.m_end_time_stamp)
        result = self.verify_del_response_msg(req_url, expect_dict)
...
#        self.test_usage_trim_with_start_and_end_and_remove_all()
    ...
    def test_end(self):
        ...
        and self.clean_env("zvampirem:swiftchosenone") \
        and self.clean_env("zvampirem:swiftchosenoneex") \
        and self.clean_env("zvampirem2") \
        and self.clean_env("zvampirem2:swiftchosenone2") \
        and self.clean_env("zvampirem3") \
        and self.clean_env("zvampirem3:swiftchosenone3"):
            print "Test End!"
